...
`GET /api/projects/:p/agents/:a/package` shows what would be published (manifest, size, whether the server can publish). `POST …/package/publish { gistId?, public? }` (owner) publishes. **An Agent keeps one gist**: the gist it was published to is recorded beside it (`.penguin-publish.json` in the Agent directory — a dotfile, so it is never packaged), and a republish updates that gist without the caller naming anything. `gistId` overrides the target; only a first publish creates. An update also *removes* what the package no longer carries — a gist update otherwise only adds and overwrites, so a renamed or deleted file would stay in the gist forever. A republish of unchanged content spends no API call at all: the digest of what was published is recorded with the gist, and a publish that would write exactly that returns `unchanged: true` without calling GitHub (naming a `gistId` always writes, which is how a gist edited or deleted on GitHub is published over). The gist's description — its title wherever GitHub lists it — is `<name> — <the Agent's own description> · PenguinHarness Agent`. If the remembered gist has since been deleted on GitHub, the next publish creates a new one instead of failing. `POST /api/agent-packages/preview { gist }` reads and validates a gist, writing nothing; `POST /api/agent-packages/install { gist, projectId, agentId }` (owner) installs it. A gist is named by its URL or bare id.
