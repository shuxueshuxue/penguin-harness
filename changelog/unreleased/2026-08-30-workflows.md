...
A page can be the whole app: `/app/:projectId/:agentId/:workflowId` shows one workflow's page with no sidebar, chat or tab strip. The tab's *Fill the app* button navigates there, the page itself can ask with `parent.postMessage({ type: "penguin:fill-app" }, "*")`, and `penguin web --app <project>/<agent>/<workflow>` opens the browser straight onto it (`Invalid --app` before anything starts when the spec is not three segments). The route has no chrome to leave by on purpose; the command palette is the way out — it now answers **Ctrl+Shift+P as well as Ctrl+P** (⌘ on macOS), so a page that takes one chord for itself cannot trap the user — and on that route carries *Exit full page (back to chat)*, which remembers the Project and Agent and lands on their chat.
